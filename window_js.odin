...
_run :: proc(title: string, size: [2]int, flags: Flags, handler: Event_Handler) {
	...
	ok := js.add_event_listener("wgpu-canvas", .Mouse_Down, nil, __mouse_down_callback)
	assert(ok)
	ok  = js.add_event_listener("wgpu-canvas", .Mouse_Up, nil, __mouse_up_callback)
	assert(ok)
	ok  = js.add_event_listener("wgpu-canvas", .Mouse_Move, nil, __mouse_move_callback)
	assert(ok)
